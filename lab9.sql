CREATE OR REPLACE FUNCTION increase_value(input_value INT) RETURNS INT AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION find_employee(emp_name TEXT) RETURNS TABLE(
    employee_id INT, name TEXT, position TEXT, salary NUMERIC, department TEXT
) AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION list_products(product_category TEXT) RETURNS TABLE(
    product_id INT, product_name TEXT, price NUMERIC, stock INT
) AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION update_salary(emp_id INT) RETURNS VOID AS $$
DECLARE
